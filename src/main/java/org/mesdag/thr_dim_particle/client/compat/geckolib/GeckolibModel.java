...
public class GeckolibModel {
        ...
        for (GeoCube cube : bone.getCubes()) {
            ...
            for (GeoQuad quad : cube.quads()) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    int u = Float.floatToIntBits(sprite.getU(vertex.texU()));
                    int v = Float.floatToIntBits(sprite.getV(vertex.texV()));
    ...
    private void l(TDParticle particle, Matrix4f pose, ParticleBuffer buffer, float vx, float vy, float vz) {
        ...
        for (CompiledVertex[] quad : quads) {
            ...
            float y02 = y2 - y0;
            float z02 = z2 - z0;
            if (Math.fma(vx, Math.fma(y01, z02, -z01 * y02), Math.fma(vy, Math.fma(z01, x02, -x01 * z02), vz * Math.fma(x01, y02, -y01 * x02))) < 0) {
